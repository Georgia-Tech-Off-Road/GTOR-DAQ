...
//save a wire object and use that for transmission to allow pins to be switched

class ADS1115 {
    public:
        ADS1115(uint8_t alert, uint8_t address, adsGain_t gain, uint16_t dataRate, I2CDriverWire* deviceWire);
        uint16_t* getAnalogReadings();
    ...
    private:
        ...
        static uint16_t _dataReadings[4];
        static I2CDriverWire* _deviceWire;
