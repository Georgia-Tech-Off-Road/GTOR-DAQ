...
TEST(SensorDataTests, Initialize) {
    SensorData<std::tuple<cmbtl::microsec::MICRO_SEC_SENSOR_INFO>> sensorData;
}

TEST(SensorDataTests, SensorAt) {
    ASSERT_TRUE((std::is_same<typename SensorDataType::SensorAt<0>, cmbtl::microsec::MICRO_SEC_SENSOR_INFO>::value));
}

TEST(SensorDataTests, SVTypeAt) {
    ASSERT_TRUE((std::is_same<typename SensorDataType::SVTypeAt<0>, cmbtl::microsec::MICRO_SEC_SENSOR_INFO::STORED_VALUE>::value));
}

TEST(SensorDataTests, SVTupleType) {
    ASSERT_TRUE((std::is_same<std::tuple<cmbtl::microsec::MICRO_SEC_SENSOR_INFO::STORED_VALUE>, 
...
TEST(SensorDataTests, getData) {
    ...
    ASSERT_TRUE((std::is_same<decltype(sensorData.getData<0>()),cmbtl::microsec::MICRO_SEC_SENSOR_INFO::STORED_VALUE>::value));
}

TEST(SensorDataTests, getRefToData) {
    SensorDataType sensorData;

    cmbtl::microsec::MICRO_SEC_SENSOR_INFO::STORED_VALUE& millisecRef = sensorData.getRefToData<0>();
    millisecRef = 42;
...
TEST(SensorDataTests, getConstRefToData) {
    ...
    cmbtl::microsec::MICRO_SEC_SENSOR_INFO::STORED_VALUE const  &constMillisecRef = sensorData.getConstRefToData<0>();
...
TEST(SensorDataTests, encodedSizeTable) {
    ...
    std::array<uint32_t, SensorDataType::NUM_SENSORS> expected = {cmbtl::microsec::MICRO_SEC_SENSOR_INFO::ENCODED_BIT_SIZE};
...
TEST(SensorDataTests, runtimeDecodeData) {
    SensorDataType sensorData;
    BinaryBuffer buffer(cmbtl::microsec::MICRO_SEC_SENSOR_INFO::ENCODED_BIT_SIZE);
    buffer.writeValue<cmbtl::microsec::MICRO_SEC_SENSOR_INFO::STORED_VALUE>(359, cmbtl::microsec::MICRO_SEC_SENSOR_INFO::ENCODED_BIT_SIZE);
    sensorData.decodeSensorData<0>(buffer);
    cmbtl::microsec::MICRO_SEC_SENSOR_INFO::STORED_VALUE actual = sensorData.getData<0>();
...
TEST(SensorDataTests, convertData) {
    SensorData<std::tuple<cmbtl::microsec::MICRO_SEC_SENSOR_INFO, TestSensorInfo>> sensorData;
